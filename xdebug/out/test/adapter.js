...
(0, mocha_1.describe)('PHP Debug Adapter', () => {
    ...
    (0, mocha_1.describe)('socket path listen', () => {
        ...
        (process.platform === 'win32' ? mocha_1.it.skip : mocha_1.it)('should error on existing unix pipe', async () => {
            await assert.isRejected(client.launch({
                program,
                hostname: 'unix:///tmp',
    ...
    (0, mocha_1.describe)('evaluation', () => {
        ...
        (0, mocha_1.it)('should return the eval result for clipboard', async () => {
            const program = path.join(TEST_PROJECT, 'variables.php');
            await client.launch({
                program,
            });
            ...
            assert.equal(response.result, '123');
            assert.equal(response.variablesReference, 0);
            const response2 = (await client.evaluateRequest({
                context: 'clipboard',
                frameId: frame.id,
                expression: '$aString',
            })).body;
            ...
            const response3 = (await client.evaluateRequest({
                context: 'clipboard',
                frameId: frame.id,
                expression: '$anArray',
            })).body;
            assert.equal(response3.result, 'array (\n  0 => 1,\n  test => 2,\n  test2 => \n  array (\n    t => 123,\n  ),\n)');
    ...
    (0, mocha_1.describe)('stream tests', () => {
        const program = path.join(TEST_PROJECT, 'output.php');
        (0, mocha_1.it)('listen with externalConsole', async () => {
