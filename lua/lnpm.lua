...
local function install_plugin(repo, opts, callback)
  ...
  local install_dir = config.install_path .. plugin_name
  local repo_url = 'https://github.com/' .. repo .. '.git'
  
  -- Создаем директорию если не существует
  ensure_dir(config.install_path)
...
function M.list()
  ...
  local handle = io.popen('ls -1 "' .. config.install_path .. '" 2>/dev/null')
  if handle then
...
function M.update()
  ...
  local plugins_dir = config.install_path
